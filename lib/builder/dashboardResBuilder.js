...
(function (dashboardResBuilder) {
    ...
    dashboardResBuilder.transformInvDetailResponse = function(data, groupBy) {
        ...
        var sortBy = groupBy == 'loc' ? 'lcnm' : 'mnm';
        resmodel.n = this.sortData(resmodel.n, 'nc', sortBy);
        resmodel.mn = this.sortData(resmodel.mn, 'lmnc', sortBy);
        resmodel.mx = this.sortData(resmodel.mx, 'gmc', sortBy);
        resmodel.so = this.sortData(resmodel.so, 'soc', sortBy);
        return resmodel;
    };

    dashboardResBuilder.sortData = function (data, eventType, sortBy) {
        if (data.length > 0) {
