...
(function (assetRespBuilder) {
                    ...
                    if (assets.dId == data.dId) {
                        ...
                        if(!utils.checkNullEmptyObject(data.rel)) {
                            for (var v in data.rel) {
                                ...
                                var subrelObj = {};
                                if(utils.checkNotNullEmpty(relObj.mpId)) {
                                    subrelObj.monitoring_point = relObj.mpId;
                                }
                                subrelObj.sensor = relObj.sId;
                                subrelObj.related_asset = {};
                                subrelObj.related_asset.asset_type = relObj.typ;
                                if(constants.const.TEMPERATURE_LOGGER == relObj.typ) {
                                    subrelObj.related_asset.asset_type = subrelObj.related_asset.asset_type + 1;
                                }

                                subrelObj.related_asset.monitoring_type = undefined;
                                if (utils.checkNotNullEmpty(relObj.meta.dev.mdl)) {
                                    subrelObj.related_asset.model = relObj.meta.dev.mdl;
                                }
                                subrelObj.related_asset.serial_no = relObj.dId;
                                subrelObj.related_asset.manufacturer_code = relObj.vId;
                                assets.related_assets.push(subrelObj);
                            }
                        }
                        assets.et = {};
                        ...
                        data.alrm.forEach(function (alrmdata) {
                            ...
                            } else if(alrmdata.typ == constants.const.TYPE_ACTIVITY && utils.checkNullEmpty(alrmdata.mpId)) {
                                if (alrmdata.stat == constants.const.STATUS_KNOWN) {
                                    assetActiveData[constants.const.MPID_OALL] = true;
                                } else {
                                    assetActiveData[constants.const.MPID_OALL] = false;
                                    inactiveSinceData[constants.const.MPID_OALL] = alrmdata.time;
                                }
                            }
                        });
                        var i = 0;
                        if (data.tmp != null) {
                            data.tmp.forEach(function (tempdata) {
                                tempdata.iA = assetActiveData[tempdata.mpId] != undefined ? assetActiveData[tempdata.mpId] : assetActiveData[constants.const.MPID_OALL];
                                if (tempdata.mpId == undefined) {
                                    assets.temperature_status = tempdata.st;
                                    assets.abnormality_type =  tempdata.aSt;
                                    assets.activity_status = tempdata.iA;
                                    i++;
                                } else {
                                    if (tempdata.iA) {
                        ...
                        if(!utils.checkNullEmptyObject(data.rel)) {
                            assets.is_monitored = true;
                        } else {
                            assets.is_monitored = false;
                        }
                        delete assets.iDa;
                        delete assets.sno;
                    }
        ...
        if (alertData) {
            alertData.data.forEach(function (assetData) {
                if(assetData.tmpalm != undefined) {
                    var deviceModel = assetData.tmpalm;
    ...
    assetRespBuilder.buildAssetDetailModel = function (data) {
        ...
        if(!utils.checkNullEmptyObject(data.meta.cc)) {
            var capacity = data.meta.cc.split(" ");
            model.capacity = {};
            model.capacity.quantity = capacity[0];
            model.capacity.units = capacity[1];
            //new field
            model.capacity_new= data.meta.cc;
        }
        model.owners = [];
        ...
        if(data.mts.length > 0) {
            data.mts.forEach(function(entry){
                var usr = {};
                usr.id = entry.id;
                usr.name = entry.fnm;
                usr.phone = entry.phm;
        ...
        //entity detail
        model.entity = {};
        if (!utils.checkNullEmptyObject(data.entity)) {
            model.entity.entity_id = data.entity.id;
            model.entity.name = data.entity.nm;
            model.entity.country = data.entity.ctr;
            model.entity.state = data.entity.st;
            model.entity.district = data.entity.ds;
            model.entity.city = data.entity.ct;
            model.entity.taluk = data.entity.tlk;
        }
        return model;
    };
})(module.exports);
