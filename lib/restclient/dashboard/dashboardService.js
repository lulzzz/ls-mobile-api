...
(function (dashboardService) {
    ...
    dashboardService.getActivityDashboard = function(queryModel, callback) {
        var options = {
            url: config.baseurl + config.dashconfig.dash_act.url,
            method: config.dashconfig.dash_act.method,
            headers: {
                'Accept-Charset': 'utf-8',
                'Content-Type': 'application/json',
            ...
            qs: {
                incetags: queryModel.incetags,
                exetags: queryModel.exetags,
                mtags: queryModel.mtags,
                mnm: queryModel.mnm,
                loc: queryModel.loc,
                locty: queryModel.locty,
                p: queryModel.p,
                date: queryModel.date,
                refresh: queryModel.refresh
            },
            timedOut: config.dashconfig.dash_act.timeout
        };
        restClient.callApi(options, callback);
    };
